...
with open(path, 'w') as f:
    f.write(r"""import { useState, useEffect, useRef } from 'react'
...
async function callAI({ system, messages, maxTokens = 1024 }) {
  ...
  console.log('[callAI] key present:', !!key, 'length:', key ? key.length : 0)
...
function ClientBriefTool() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('[ClientBriefTool] error:', err)
  ...
  const send = async () => {
    ...
    try {
      const reply = await callAI({
        system: `You are a research assistant for a professional travel advisor. Answer questions about European destinations, restaurants, hotels, logistics, and trip planning. Be specific and direct. Write for an experienced advisor. No filler, no tourism cliches.`,
        messages: updated,
      })
      setMessages([...updated, { role: 'assistant', content: reply }])
    } catch (err) {
      console.error('[ResearchTool] error:', err)
